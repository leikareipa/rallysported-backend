...
{
    k_assert((CURRENT_VIDEO_MODE == VIDEO_MODE_GRAPHICS), "Can only set the palette while in graphics mode.");

    k_assert((paletteIdx < 5), "Palette index out of bounds.");
...
void krender_clear_surface(void)
{
    switch (CURRENT_VIDEO_MODE)
    {
        ...
        default: k_assert(0, "Unknown video mode.");
    }
...
int krender_enter_grapics_mode(void)
{
    ...
    #else
        sdlWindow = SDL_CreateWindow("RallySportED-DOS - RGEO", 0, 0, 1280, 800, SDL_WINDOW_OPENGL);
