...
struct mesh_s load_prop_mesh(const int propType)
{
    ...
    switch (propType)
    {
        ...
        default: k_assert(0, "Unknown prop type."); break;
    }
    ...
    k_assert(vertexCoords, "Failed to properly load prop mesh vertex coordinates.");
...
{
    k_assert(((propType >= 0) && (propType <= PROP_TYPE_COUNT)), "Accessing props out of bounds.");
