                                                          ...
                                                          const uint32_t elementByteSize)
{
    ...
    k_assert(newStack, "Failed to allocate memory for a new stack.");
                               ...
                               uint32_t newElementCount)
{
    ...
    k_assert(stack, "Attempting to operate on a NULL stack.");
    
    k_assert((newElementCount > stack->capacity) &&
             (newElementCount > stack->count),
             "Trying to grow the stack to a smaller size than what it is now.");

    k_assert((stack->count <= stack->capacity),
             "Attempting to grow a malformed stack.");

    newStackBuffer = calloc(newElementCount, stack->elementByteSize);
    k_assert(newStackBuffer, "Failed to allocate memory to grow the stack.");
                                    ...
                                    const void *const newElement)
{
    ...
    k_assert((stack->count < stack->capacity), "Failed to properly grow the stack.");
...
{
    k_assert((stack->count > 0), "Attempting to pop an empty stack.");
...
void* kelpo_generic_stack__at(const struct kelpo_generic_stack_s *const stack,
                              const uint32_t idx)
{
    k_assert((idx < stack->count), "Attempting to access the stack out of bounds.");
